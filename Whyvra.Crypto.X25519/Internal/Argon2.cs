...
using Whyvra.Crypto.X25519.Memory;

namespace Whyvra.Crypto.X25519.Internal
{
    internal class Argon2 : IDisposable
    {
        ...
        private readonly Blocks _memoryBlocks;
        private readonly int _memoryBlockCount;
        ...
        public Argon2(PinnedMemory<byte> key, byte[] salt, byte[] associatedData = null)
        {
            ...
            var memoryBlocks = (uint)MemoryCost;
            if (memoryBlocks < 2 * _syncPoints * Lanes)
            {
                memoryBlocks = 2 * _syncPoints * (uint)Lanes;
            }

            _segmentLength = (int)(memoryBlocks / (Lanes * _syncPoints));
            _laneLength = _segmentLength * _syncPoints;
            _memoryBlockCount = _laneLength * Lanes;
        ...
        private PinnedMemory<byte> InitialHash()
        {
            ...
            hash.UpdateBlock(value, 0, 4);
            Store32(value, MemoryCost);
            hash.UpdateBlock(value, 0, 4);
            Store32(value, TimeCost);
            hash.UpdateBlock(value, 0, 4);
        ...
        private void FillMemoryBlocks()
        {
            if (Threads > 1)
            {
                var waitHandles =
                    Enumerable.Range(
                        0,
                        Threads > Lanes ? Lanes : Threads)
                        .Select(_ => new AutoResetEvent(false))
                        .Cast<WaitHandle>()
                        .ToArray();
                var threads = new Thread[waitHandles.Length];
                for (var passNumber = 0; passNumber < TimeCost; ++passNumber)
                {
                    for (var sliceNumber = 0; sliceNumber < _syncPoints; ++sliceNumber)
                    {
                        var laneNumber = 0;
                        var remaining = Lanes;
                        for (; laneNumber < threads.Length && laneNumber < Lanes; ++laneNumber)
                        {
            ...
            else
            {
                for (var passNumber = 0; passNumber < TimeCost; ++passNumber)
                {
                    for (var sliceNumber = 0; sliceNumber < _syncPoints; ++sliceNumber)
                    {
                        for (var laneNumber = 0; laneNumber < Lanes; ++laneNumber)
                        {
                            FillSegment(
                                new Position
                                {
        ...
        private void FillSegment(Position position)
        {
            ...
            for (var i = startingIndex; i < _segmentLength; ++i, ++curOffset, ++prevOffset)
            {
                ...
                var refLane =
                    ...
                    ? position.Lane
                    : (int)((uint)(pseudoRand >> 32) % (uint)Lanes);
        ...
        private void Blake2BLong(byte[] hash, byte[] inbuf)
        {
            ...
            using (var blakeHash = new Blake2b())
            {
